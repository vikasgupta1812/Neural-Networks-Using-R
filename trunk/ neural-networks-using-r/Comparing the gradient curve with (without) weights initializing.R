# Comparison of the training between MLPs with and without the 
# "Linear-Least-Squares Initialization of  Multilayer Perceptrons Through 
# Backpropagation of the Desired Response  Deniz et al (2005), 
# IEEE Transactions on Neural Networks, 16-2.
# 
...
InitialWOH<-WeightsOH # storing
InitialWHI<-WeightsHI # storing
N<-length(T) # number of samples
Lr<-0.2 # learning rate
Mr<-0.15 # momentum rate
NumEpochs<-1000
MSE<-rep(0,NumEpochs)
MSE2<-rep(0,NumEpochs)
...
WeightsOH<-InitialWOH# restoring
WeightsHI<-InitialWHI# restoring
...
for (epoch in 1:NumEpochs){
	...
	SumOutput<-WeightsOH%*%OutputInputs
	Outputs<-phi(SumOutput)
	MSE2[epoch]<-as.numeric(t(Targets-t(Outputs))%*%(Targets-t(Outputs)))/(2*N)
	...
	Grad<-c(DeltaWOH,DeltaWHI)/Lr
	GradNorm2[epoch]<-sqrt(t(Grad)%*%Grad)
	WeightsOH<-WeightsOH+DeltaWOH+Mr*PreviousDeltaWOH
	WeightsHI<-WeightsHI+DeltaWHI+Mr*PreviousDeltaWHI
	if(epoch%%100==0){
		cat("Epoch: ");cat(epoch);cat(" | MSE: ");cat(MSE2[epoch]);cat(" | Gradient: ");cat(GradNorm2[epoch]);cat("\n");
	}
}
par(mfrow=c(2,1))
